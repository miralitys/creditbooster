...
function getLeadColumnNames(db) {
  return db
...
function initLeadsDb(dbPath) {
  ...
  const existingColumns = new Set(getLeadColumnNames(db));
  const utmColumns = [
    'utm_source',
    'utm_medium',
    'utm_campaign',
...
function insertLead(dbPath, lead) {
  ...
  const stmt = db.prepare(`
    INSERT INTO leads (
      created_at, full_name, email, phone, source, page_slug, page_url, page_title, ip, user_agent,
      utm_source, utm_medium, utm_campaign, utm_content, utm_term, crm_status, crm_error
    ) VALUES (
      @created_at, @full_name, @email, @phone, @source, @page_slug, @page_url, @page_title, @ip, @user_agent,
      @utm_source, @utm_medium, @utm_campaign, @utm_content, @utm_term, @crm_status, @crm_error
    )
